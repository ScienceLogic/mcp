...
class StripUnknownArgumentsMiddleware(Middleware):
    ...
    async def on_call_tool(self, context, call_next):
        """Filter out unknown arguments from tool calls.  Looking at clients like you N8N: https://github.com/n8n-io/n8n/issues/21716"""
        try:
            ...
            if context.fastmcp_context and context.message.arguments and len(context.message.arguments) > 0:
                ...
                if unknown_args:
                    logger.info("Unknown arguments for tool %s: %s", context.message.name, list(unknown_args))
                context.message.arguments = filtered_args  # modify in place
        except Exception as e: 
            logger.error(
                "Error in %s: %s",
                StripUnknownArgumentsMiddleware.__name__,
                e,
                exc_info=True,
            )
